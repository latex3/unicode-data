...
% Write some basic information to the log.
  ...
  \newlinechar=`\^^J %
  \message{^^J}%
  \message{load-unicode-casing.tex v0.4 (2015-12-07)^^J}%
  \message{Reading Unicode casing and letter data^^J}%
...
% From plain: may not be defined (yet).
  ...
  \let\repeat\fi
% Force the full path of the data file to appear in the log.
  \catcode`\#=12 %
  \everyeof{\noexpand}%
  \edef\temp{\input UnicodeData.txt }%
  \message{^^J}%
% There is no version data in |UnicodeData.txt| so a |.def| file is provided
% as part of the bundle.
  \openin0=UnicodeData.def %
  \read0 to \unicodedataline
