...
%
% From the files EastAsianWidth.txt and LineBreak.txt inter character
% classes for XeTeX (\XeTeXcharclass) are set for the following classes
% of code point
% - "ID" (ideographic)
% - "OP" (opener)
% - "CL" (closer)
...
% Write some basic information to the log.
  ...
  \newlinechar=`\^^J %
  \message{^^J}%
  \message{load-unicode-data.tex v0.5 (2015-12-08)^^J}%
...
% First |MathClass.txt|: this just needs one token of the 'rest' of the line.
  ...
  \def\parseunicodedataIV#1#2#3 #4\relax{%
    ...
    \else
      ...
      \loop
        \ifnum\count0<"#2 %
          \advance\count0 by 1 %
          \expandafter\def\csname EAW@\number\count0\endcsname{#3}%
      \repeat
    \fi
  }%
  \readandparse{EastAsianWidth}%
% Now for |LineBReak.txt|: set up the different line break classes recognised.
  ...
  \def\parseunicodedataIV#1#2#3 #4\relax{%
    ...
    \else
      \ifnum 0%
        ...
        \if W\csname EAW@\number"#1\endcsname 1\fi
         >0 %
       \global\XeTeXcharclass"#1=\csname XeTeXcharclass#3\endcsname
      \fi
    \fi
  }%
% If XeTeX is not in use, simply make the final stage of the parser a no-op.
% This way the same files are always read.
